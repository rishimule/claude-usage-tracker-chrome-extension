...
11. **Never delete branches after merging.** Do not pass `--delete-branch` to `gh pr merge` and do not run `git branch -d` / `git push origin --delete` on merged branches. Only delete a branch when the user explicitly asks for it.
